...
// MRI include ast logger and options
...
static void conference_exec( struct ast_conference *conf )
{
	...
	ast_debug(1, "Entered conference_exec, name => %s\n", conf->name ) ;
#ifdef	REALTIME
	int policy;
	...
	while ( 42 == 42 )
	{
		...
		// ast_debug(1, "PROCESSING FRAMES, conference => %s, step => %d, ms => %ld\n",
		...
		// ast_debug(1, "begin processing incoming audio, name => %s\n", conf->name ) ;
		...
		// ast_debug(1, "finished processing incoming audio, name => %s\n", conf->name ) ;
...
//			ast_log ( LOG_WARNING, "base = %d,%d: conf->delivery_time = %d,%d\n",base.tv_sec,base.tv_usec, conf->delivery_time.tv_sec, conf->delivery_time.tv_usec);
	...
	ast_debug(1, "exit conference_exec\n" ) ;
...
struct ast_conference* join_conference( struct ast_conf_member* member, char* max_users_flag )
{
	...
	ast_debug(1, "attempting to find requested conference\n" ) ;
	...
	if ( conf == NULL )
	{
		...
		ast_debug(1, "attempting to create requested conference\n" ) ;
...
static struct ast_conference* find_conf( const char* name )
{
	...
	if ( conflist == NULL )
	{
		ast_debug(1, "conflist has not yet been initialized, name => %s\n", name ) ;
		return NULL ;
	}
	...
	while ( conf != NULL )
	{
		if ( strncasecmp( (char*)&(conf->name), name, 80 ) == 0 )
		{
			...
			ast_debug(1, "found conference in conflist, name => %s\n", name ) ;
			return conf;
		}
		conf = conf->next ;
	}

	ast_debug(1, "unable to find conference in conflist, name => %s\n", name ) ;
	return NULL;
}
...
static struct ast_conference* create_conf( char* name, struct ast_conf_member* member )
{
	ast_debug(1, "entered create_conf, name => %s\n", name ) ;
	...
	ast_debug(1, "added new conference to conflist, name => %s\n", name ) ;
	...
	if ( ast_pthread_create( &conf->conference_thread, NULL, (void*)conference_exec, conf ) == 0 )
	{
		...
		ast_debug(1, "started conference thread for conference, name => %s\n", conf->name ) ;
	}
	else
	{
...
static void remove_conf( struct ast_conference *conf )
{
  int c;
	...
	// ast_debug(1, "attempting to remove conference, name => %s\n", conf->name ) ;
	...
	while ( conf_current != NULL )
	{
		...
		if ( conf_current == conf )
		{
			...
			if (conf->debug_flag)
			{
				...
				ast_debug(1, "removed conference, name => %s\n", conf_current->name ) ;
			}
...
static void add_member( struct ast_conf_member *member, struct ast_conference *conf )
{
	...
	ast_debug(1, "member added to conference, name => %s\n", conf->name ) ;
...
void remove_member( struct ast_conf_member* member, struct ast_conference* conf )
{
	...
	ast_debug(1, "removed member from conference, name => %s, remaining => %d\n",
	...
	if ( member->bucket != NULL )
	{
		...
		//ast_debug(1, "Removed %s from the channel table, bucket => %ld\n", member->chan->name, member->bucket - channel_table ) ;
	}
	...
	ast_log(LOG_NOTICE, "Channel %s removed from conference %s (Member: %d)\n", member->chan->name, member->conf_name, member->id);
...
int show_conference_stats ( int fd )
{
	...
	if ( conflist == NULL )
	{
		ast_debug(1, "conflist has not yet been initialized.\n") ;
		return 0 ;
	}
...
int get_conference_stats( ast_conference_stats* stats, int requested )
{
	...
	if ( conflist == NULL )
	{
		ast_debug(1, "conflist has not yet been initialize\n" ) ;
		return 0 ;
	}
...
int hash(const char *channel_name)
{
	...
	//ast_debug(1, "Hashed channel_name: %s to 0x%08x\n", channel_name, h);
	return h;
}
