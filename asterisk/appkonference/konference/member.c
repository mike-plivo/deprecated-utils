 ...
 * Copyright (C) 2001, 2006 HorizonWimba, Inc.
 ...
 * Copyright (C) 2001, 2005 Vipadia Limited
 *
...
// MRI include time functions
#include "asterisk/time.h"

// MRI include debug logger
#include "astlogger.h"
...
static int process_incoming(struct ast_conf_member *member, struct ast_conference *conf, struct ast_frame *f)
{
		...
		)
	{
		...
		//ast_debug(10, "Listen only user frame");
		ast_frfree( f ) ;
		f = NULL ;
	}
	else if ( f->frametype == AST_FRAME_VOICE )
	{	//ast_debug(10, "Got voice frame");
			...
			)
		{
			...
			if ( spx_ret == 0 )
			{
				...
				{
					// ast_debug(10, "ignore_speex_count => %d\n", ignore_speex_count ) ;
...
static int process_outgoing(struct ast_conf_member *member)
{
	...
	for(;;)
	{
		...
		{
			ast_debug(10, "unanticipated delivery time, delivery_diff => %d, delivery.tv_usec => %ld\n",
		...
		if ( ast_write( member->chan, f ) == 0 )
		{
			...
			ast_debug(10, "SENT VOICE FRAME, channel => %s, frames_out => %ld, s => %ld, ms => %ld\n",
	...
	for(;;)
	{
		...
		{
			struct timeval tv = ast_tvnow();
			ast_debug(10, "SENT VIDEO FRAME, channel => %s, frames_out => %ld, s => %ld, ms => %ld\n",
				 member->channel_name, member->frames_out, tv.tv_sec, tv.tv_usec ) ;
		}
		else
		{
			...
			// log 'dropped' outgoing frame
			ast_debug(10, "unable to write video frame to channel, channel => %s\n", member->channel_name ) ;
	...
	for(;;)
	{
		...
		{
			struct timeval tv = ast_tvnow();
			ast_debug(10, "SENT DTMF FRAME, channel => %s, frames_out => %ld, s => %ld, ms => %ld\n",
				 member->channel_name, member->frames_out, tv.tv_sec, tv.tv_usec ) ;
		...
		else
		{
			...
			// log 'dropped' outgoing frame
			ast_debug(10, "unable to write dtmf frame to channel, channel => %s\n", member->channel_name ) ;
	...
	for(;;)
	{
		...
		{
			struct timeval tv = ast_tvnow();
			ast_debug(10, "SENT TEXT FRAME, channel => %s, frames_out => %ld, s => %ld, ms => %ld\n",
				 member->channel_name, member->frames_out, tv.tv_sec, tv.tv_usec ) ;
		...
		else
		{
			...
			// log 'dropped' outgoing frame
			ast_debug(10, "unable to write text frame to channel, channel => %s\n", member->channel_name ) ;
...
//	start = ast_tvnow();
	...
	ast_debug(10, "Begin processing member thread, channel => %s\n", chan->name ) ;
...
//	ast_debug(10, "creating new member, id => %s, flags => %s, p => %s\n",
//		id, flags, priority ) ;
	...
	//
#if 0
	ast_debug(10, "CHANNEL INFO, CHANNEL => %s, DNID => %s, CALLER_ID => %s, ANI => %s\n",
		chan->name, chan->dnid, chan->callerid, chan->ani ) ;

	ast_debug(10, "CHANNEL CODECS, CHANNEL => %s, NATIVE => %d, READ => %d, WRITE => %d\n",
	...
	if ( member->spyee_channel_name != NULL )
	{
		...
		} else
		{
			...
			//ast_debug(10, "Failed to start spyer %s, spyee is %s\n", member->channel_name, member->spyee_channel_name) ;
	...
	//ast_debug(10, "Added %s to the channel table, bucket => %ld\n", member->chan->name, member->bucket - channel_table) ;
	ast_log(LOG_NOTICE, "Added %s to conference %s (Member:%d Flags:%s)\n", member->chan->name, conf->name, member->id, member->flags);
	...
	ast_debug(10, "begin member event loop, channel => %s\n", chan->name ) ;
	...
	while ( 42 == 42 )
	{
		...
		//ast_debug(10, "received event on channel, name => %s, left => %d\n", chan->name, left ) ;
	...
	ast_debug(10, "end member event loop, time_entered => %ld\n", member->time_entered.tv_sec ) ;
...
//	ast_debug(10, "expected_frames => %d\n", expected_frames ) ;
	...
	if ( spyee != NULL )
	{
		...
		//ast_debug(10, "End spyer %s, spyee is %s\n", member->channel_name, member->spyee_channel_name) ;
	}
...
struct ast_conf_member* create_member( struct ast_channel *chan, const char* data )
{
	...
	ast_debug(10, "attempting to parse passed params, stringp => %s\n", stringp ) ;
	...
	while ( (token = strsep(&stringp, argument_delimiter )) != NULL )
	{
		...
		} else if ( strncasecmp(key, arg_vad_prob_continue, sizeof(arg_vad_prob_continue) - 1) == 0 )
		{
			...
			ast_debug(1, "vad_prob_continue = %f\n", member->vad_prob_continue);
#ifdef	VIDEO
		} else if ( strncasecmp(key, arg_video_start_timeout, sizeof(arg_video_start_timeout) - 1) == 0 )
		{
			...
			ast_debug(1, "video_start_timeout = %d\n", member->video_start_timeout);
		...
		} else if ( strncasecmp(key, arg_chanspy, sizeof(arg_chanspy) - 1) == 0 )
		{
			...
			ast_debug(1, "spyee channel name is %s\n", member->spyee_channel_name);
		} else
		{
	...
	if ( member->via_telephone == 1)
	{
		...
		{
			ast_debug(10, "member dsp initialized, channel => %s, v => %d, d => %d, a => %d\n",
			...
			ast_debug(10, "speech_prob_start => %f, speech_prob_continue => %f\n",
	...
	ast_debug(10, "AST_FORMAT_SLINEAR => %d\n", AST_FORMAT_SLINEAR ) ;
	...
	ast_debug(10, "created member, type => %s, priority => %d, readformat => %d\n",
...
struct ast_conf_member* delete_member( struct ast_conf_member* member )
{
	...
	{
		// !!! DEBUGING !!!
		ast_debug(10, "freeing member flags, name => %s\n",
			member->channel_name ) ;
		free( member->flags ) ;
	}
	...
	// !!! DEBUGING !!!
	ast_debug(10, "deleting member input frames, name => %s\n",
		member->channel_name ) ;
...
#endif
	// !!! DEBUGING !!!
	ast_debug(10, "deleting member output frames, name => %s\n",
		member->channel_name ) ;
	...
	{
		// !!! DEBUGING !!!
		ast_debug(10, "destroying member preprocessor, name => %s\n",
			member->channel_name ) ;
	...
	// !!! DEBUGING !!!
	ast_debug(10, "freeing member translator paths, name => %s\n",
		member->channel_name ) ;
	...
	// !!! DEBUGING !!!
	ast_debug(10, "freeing member channel name, name => %s\n",
		member->channel_name ) ;
...
conf_frame* get_incoming_frame( struct ast_conf_member *member )
{
	...
	if ( member->inFramesCount == 0 )
	{
		...
		{
			ast_debug(10, "repeating cached frame, channel => %s, inFramesRepeatLast => %d\n",
	...
	{
		ast_debug(10, "enabling cached frame, channel => %s, incoming => %d, outgoing => %d\n",
...
{
  //ast_debug(10, "queue incoming video frame\n");
...
*/
	...
	if (member->inSmoother == NULL ){
		...
		{
			ast_debug(10, "resetting smooth_size_in. old size=> %d, multiple =>%d, datalen=> %d\n", member->smooth_size_in, member->smooth_multiple, fr->datalen );
			...
			//	ast_debug(1,"smooth_multiple does not divide datalen. changing smooth size from %d to %d, multiple => 1\n", member->smooth_size_in, fr->datalen);
				member->smooth_size_in = fr->datalen;
				member->smooth_multiple = 1;
			} else {
				// assume a fixed multiple, so divide into datalen.
				int newsmooth = fr->datalen / member->smooth_multiple ;
			//	ast_debug(1,"datalen is divisible by smooth_multiple, changing smooth size from %d to %d\n", member->smooth_size_in, newsmooth);
				member->smooth_size_in = newsmooth;
			}
...
ast_debug(10, "SMOOTH:Feeding frame into inSmoother, timestamp => %ld.%ld\n", fr->delivery.tv_sec, fr->delivery.tv_usec);
...
conf_frame* get_outgoing_frame( struct ast_conf_member *member )
{
	...
	// ast_debug(10, "getting member frames, count => %d\n", member->outFramesCount ) ;
...
{
	// ast_debug(10, "sending state change notification\n" ) ;
	...
	while ( member != NULL )
	{
		...
		if ( member->speaking_state_notify )
		{
			...
			ast_debug(10, "member state changed, channel => %s, state => %d, incoming => %d, outgoing => %d\n",
...
)
{
  int c;
	...
	if ( conf == NULL )
	{
		ast_debug(10, "unable to queue silent frame for null conference\n" ) ;
		return -1 ;
	}

	if ( member == NULL )
	{
		ast_debug(10, "unable to queue silent frame for null member\n" ) ;
		return -1 ;
	}
...
int increment_speaker_count(struct ast_conf_member *member, int lock)
{
	...
	ast_debug(10, "Increment speaker count: id=%d, count=%d\n", member->id, member->speaker_count);
...
int decrement_speaker_count(struct ast_conf_member *member, int lock)
{
	...
	ast_debug(10, "Decrement speaker count: id=%d, count=%d\n", member->id, member->speaker_count);
	...
	)
{
	...
	{
		ast_debug(10, "channel => %s, inFramesNeeded => %d, inFramesCount => %d\n",
