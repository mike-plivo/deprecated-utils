...
conf_frame* mix_single_speaker( conf_frame* frames_in, int volume )
{
	...
	// ast_debug(5, "returning single spoken frame\n" ) ;
	...
	if ( frames_in == NULL )
	{
		ast_debug(5, "unable to mix single spoken frame with null frame\n" ) ;
		return NULL ;
	}

	if ( frames_in->fr == NULL )
	{
		ast_debug(5, "unable to mix single spoken frame with null data\n" ) ;
		return NULL ;
	}

	if ( frames_in->member == NULL )
	{
		ast_debug(5, "unable to mix single spoken frame with null member\n" ) ;
		return NULL ;
	}
...
)
{
	...
	{
		ast_debug(5, "passed spoken frame list was NULL\n" ) ;
		return NULL ;
	}
	...
	if ( speakers < 2 )
	{
		ast_debug(5, "mix_multiple_speakers() called with less than two speakers\n" ) ;
		return NULL ;
	}
#endif // APP_KONFERENCE_DEBUG
	...
	while ( cf_spoken != NULL )
	{
		...
		{
			// ast_debug(5, "converting frame to slinear, channel => %s\n", cf_spoken->member->channel_name ) ;
...
conf_frame* delete_conf_frame( conf_frame* cf )
{
  int c;
	...
	if ( cf == NULL )
	{
		ast_debug(5, "unable to delete null conf frame\n" ) ;
		return NULL ;
	}
...
conf_frame* copy_conf_frame( conf_frame* src )
{
	...
	if ( src == NULL )
	{
		ast_debug(5, "unable to copy null conf frame\n" ) ;
		return NULL ;
	}
	...
	if ( cfr == NULL )
	{
		ast_debug(5, "unable to create new conf frame for copy\n" ) ;
		return NULL ;
	}
...
conf_frame* get_silent_frame( void )
{
	...
	{
		// ast_debug(5, "creating cached silent frame\n" ) ;
