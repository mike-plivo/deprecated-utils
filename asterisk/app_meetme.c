...
ASTERISK_FILE_VERSION(__FILE__, "$Revision: 236509 $")
...
enum {
	...
	ADMINFLAG_SELFBIMUTED =  (1 << 4),  /*!< User muted self for talk and listen */
	ADMINFLAG_BIMUTED =      (1 << 5)  /*!< User muted for talk and listen */
};
...
enum {
	...
	/*! Destroy conf if only one user is present after someone leave */
	CONFFLAG_QUIT_ONEUSER = (1 << 29),
	/*! Set dtmf to exit conf */
	CONFFLAG_DTMFEXIT = (1 << 30),
};
...
AST_APP_OPTIONS(meetme_opts, BEGIN_OPTIONS
	...
	AST_APP_OPTION('N', CONFFLAG_QUIT_ONEUSER ),
	AST_APP_OPTION_ARG('p', CONFFLAG_DTMFEXIT, OPT_ARG_DTMFEXIT ),
END_OPTIONS );
...
"is specified, by pressing <dtmf> set.\n"
...
"      'p(<dtmf>)'\n"
"          -- allow user to exit the conference by pressing <dtmf>\n"
...
"      'N' -- quit if only one user is in conf after an user left \n";
...
static int meetme_cmd(int fd, int argc, char **argv) 
{
	...
	} else if (strcmp(argv[1], "bimute") == 0) {
		if (argc < 4)
			return RESULT_SHOWUSAGE;
		strncat(cmdline, "|Z|", sizeof(cmdline) - strlen(cmdline) - 1);	
		strncat(cmdline, argv[3], sizeof(cmdline) - strlen(cmdline) - 1);
	} else if (strcmp(argv[1], "biunmute") == 0) {
		if (argc < 4)
			return RESULT_SHOWUSAGE;
		strncat(cmdline, "|z|", sizeof(cmdline) - strlen(cmdline) - 1);	
		strncat(cmdline, argv[3], sizeof(cmdline) - strlen(cmdline) - 1);
		...
		AST_LIST_TRAVERSE(&cnf->userlist, user, list) {
			...
			if ( !concise )
				ast_cli(fd, "User #: %-2.2d %12.12s %-20.20s Channel: %s %s %s %s %s %s %02d:%02d:%02d\n",
					...
					user->adminflags & ADMINFLAG_BIMUTED ? "(Admin BiMuted)" : user->adminflags & ADMINFLAG_SELFBIMUTED ? "(BiMuted)" : "",
				...
				ast_cli(fd, "%d!%s!%s!%s!%s!%s!%s!%s!%d!%02d:%02d:%02d\n",
					...
					user->adminflags & (ADMINFLAG_BIMUTED | ADMINFLAG_SELFBIMUTED)  ? "1" : "",
...
{
	static char *cmds[] = {"lock", "unlock", "mute", "unmute", "kick", "list", "bimute", "biunmute", NULL};
...
"Usage: meetme (un)lock|(un)mute|bi(un)mute|kick|list [concise] <confno> <usernumber>\n"
...
static int conf_run(struct ast_channel *chan, struct ast_conference *conf, int confflags, char *optargs[])
{
	...
	struct ast_conf_user *user2 = NULL;
	struct ast_conf_user *usr = NULL;
	int fd;
	...
	char *dtmfexit = NULL;
	struct dtmfs_feature *dfeature = NULL;


	if (!(user = ast_calloc(1, sizeof(*user))))
		return ret;

	if (!(dfeature = ast_calloc(1, sizeof(*dfeature))))
		return ret;
	
	dfeature->script = NULL;
	dfeature->intertimeout = 400;
	dfeature->pos = 0;

	const char *dscript = pbx_builtin_getvar_helper(chan, "MEETME_FEATURE_SCRIPT");
	const char *dpattern = pbx_builtin_getvar_helper(chan, "MEETME_FEATURE_PATTERN");
	const char *dtimeout = pbx_builtin_getvar_helper(chan, "MEETME_FEATURE_ITIME");
	const char *dextras = pbx_builtin_getvar_helper(chan, "MEETME_FEATURE_EXTRAS");
	if (!ast_strlen_zero(dscript)) {
		dfeature->script = ast_strdupa(dscript);
	}
	if (!ast_strlen_zero(dpattern)) {
		snprintf(dfeature->pattern, 256, "%s", ast_strdupa(dpattern));
	}
	if (!ast_strlen_zero(dtimeout)) {
		const char *itimeout = ast_strdupa(dtimeout);
		if (!ast_strlen_zero(itimeout)) {
			dfeature->intertimeout = atoi(dtimeout);
		}
	}
	if (!ast_strlen_zero(dextras)) {
		snprintf(dfeature->extras, 256, "%s", ast_strdupa(dextras));
	}
	dfeature->now.tv_usec = 0;
	dfeature->now.tv_sec = 0;
	dfeature->active = dtmf_feature_active(chan, dfeature);
	if (dfeature->active == 1) {
		ast_log(LOG_DEBUG, "MeetmeDtmf: Channel:%s Script:%s Intertimeout:%d Pattern:%s Extras:%s\n", chan->name, dfeature->script, dfeature->intertimeout, dfeature->pattern, S_OR(dfeature->extras, "")); 
	}
	

	/* set dtmf exit if needed */
	if ((confflags & CONFFLAG_DTMFEXIT) && !ast_strlen_zero(optargs[OPT_ARG_DTMFEXIT])) {
	       dtmfexit = ast_strdupa(optargs[OPT_ARG_DTMFEXIT]);
	}
	...
	if (confflags & CONFFLAG_AGI) {
		...
		for(;;) {
			...
			*/
			...
			if ((user->adminflags & (ADMINFLAG_MUTED | ADMINFLAG_SELFMUTED | ADMINFLAG_BIMUTED | ADMINFLAG_SELFBIMUTED)) && (ztc.confmode & DAHDI_CONF_TALKER)) {
			...
			if (!(user->adminflags & (ADMINFLAG_MUTED | ADMINFLAG_SELFMUTED | ADMINFLAG_SELFBIMUTED | ADMINFLAG_BIMUTED)) && !(confflags & CONFFLAG_MONITOR) && !(ztc.confmode & DAHDI_CONF_TALKER)) {
				ztc.confmode |= DAHDI_CONF_TALKER;
				if (ioctl(fd, DAHDI_SETCONF, &ztc)) {
					ast_log(LOG_WARNING, "Error setting conference - Un/Mute \n");
					ret = -1;
			...
			if (user->adminflags & ADMINFLAG_KICKME) {
				...
				if (!(confflags & CONFFLAG_QUIET) && 
                                    !(confflags & CONFFLAG_QUIT_ONEUSER) && 
			...
			if (c) {
				...
				if ((confflags & CONFFLAG_MONITOR) || (user->adminflags & (ADMINFLAG_MUTED | ADMINFLAG_SELFMUTED | ADMINFLAG_SELFBIMUTED | ADMINFLAG_BIMUTED)))
				...
				if (f->frametype == AST_FRAME_DTMF) {
					...
					dtmfstr[1] = '\0';
					if (dfeature->active == 1) {
						meetme_catch_dtmfs(c, conf->confno, user->user_no, dtmfstr[0], dfeature);
					}
				}
				...
				} else if ((f->frametype == AST_FRAME_DTMF) && (confflags & CONFFLAG_DTMFEXIT) && (dtmfexit)) {
					if (strchr(dtmfexit, f->subclass)) {
						ast_log(LOG_DEBUG, "Leaving conf by digit pressed '%s' \n", dtmfexit);
						ret = 0;
						ast_frfree(f);
						break;
					}
				...
				if (res > 0) {
					if (user->adminflags & (ADMINFLAG_BIMUTED | ADMINFLAG_SELFBIMUTED)) {
					    goto nowritebimuted;
					}
...
nowritebimuted:
 ...
 outrun:
	...
	if ((confflags & CONFFLAG_QUIT_ONEUSER) && (conf->users <= 1)) {
		ast_log(LOG_WARNING, "Destroying conf (quit on last one)\n");
		AST_LIST_TRAVERSE(&conf->userlist, user2, list) {
		    user2->adminflags |= ADMINFLAG_KICKME;
		}
	}
	free(user);
	free(dfeature);
...
static int admin_exec(struct ast_channel *chan, void *data) {
	...
	case 110: /* n: Unmute all users */
		AST_LIST_TRAVERSE(&cnf->userlist, user, list)
			user->adminflags &= ~(ADMINFLAG_MUTED | ADMINFLAG_SELFMUTED);
		break;
	case 90:  /* Z: Mute user on talk/listen */
		if (user) {
			user->adminflags |= ADMINFLAG_BIMUTED;
		} else
			ast_log(LOG_NOTICE, "Specified User not found!\n");
		break;
	case 122:  /* z: Unmute user on talk/listen */
		if (user) {
			user->adminflags &= ~(ADMINFLAG_BIMUTED | ADMINFLAG_SELFBIMUTED);
		} else
			ast_log(LOG_NOTICE, "Specified User not found!\n");
		break;
